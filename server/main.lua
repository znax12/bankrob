...
RegisterServerEvent('esx_bankrob:tooFar')
AddEventHandler('esx_bankrob:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_bankrob:killBlip', xPlayers[i])
		end
	end

	if robbers[_source] then
		TriggerClientEvent('esx_bankrob:tooFar', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('esx_bankrob:robberyStarted')
AddEventHandler('esx_bankrob:robberyStarted', function(currentStore)
	local _source  = source
			...
			if cops >= Config.PoliceNumberRequired then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
						...
						TriggerClientEvent('esx_bankrob:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('esx_bankrob:currentlyRobbing', _source, currentStore)
				TriggerClientEvent('esx_bankrob:startTimer', _source)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('esx_bankrob:robberyComplete', _source, store.reward)
