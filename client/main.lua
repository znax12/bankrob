...
RegisterNetEvent('esx_bankrob:currentlyRobbing')
AddEventHandler('esx_bankrob:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('esx_bankrob:killBlip')
AddEventHandler('esx_bankrob:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('esx_bankrob:setBlip')
AddEventHandler('esx_bankrob:setBlip', function(position)
...
RegisterNetEvent('esx_bankrob:tooFar')
AddEventHandler('esx_bankrob:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('esx_bankrob:robberyComplete')
AddEventHandler('esx_bankrob:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('esx_bankrob:startTimer')
AddEventHandler('esx_bankrob:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('esx_bankrob:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('esx_bankrob:tooFar', store)
			end
		end
	end
end)
